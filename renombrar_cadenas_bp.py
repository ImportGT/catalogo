...
ORIGEN = "CADENAS BP"
DESTINO = os.path.join("imagenes", "BP", "cadenasbp")
EXCEL_PATH = "CADENAS BP.xlsx"
PREFIJO_DESTINO = "cadenasbp"
...
if not os.path.exists(EXCEL_PATH):
    ...
    # Leer los IDs válidos desde el Excel (encabezado en la fila 2)
    try:
        df = pd.read_excel(EXCEL_PATH, header=2)
        ids_validos = set()
        for idx, row in df.iterrows():
            col_id = None
            for col in ['NUMERO', 'NUM.', 'ID', 'Id', 'numero', 'Número', 'CODIGO']:
                if col in df.columns:
                    col_id = col
                    break
            
            # Si no encuentra por nombre exacto, buscar en la segunda columna (índice 1)
...
if not os.path.exists(ORIGEN):
    ...
    for archivo in archivos:
        ...
        # Extraer números del nombre original del archivo (ej. "cadena_1_2.jpg" -> [1, 2])
        ...
        # Obtener el resto de la cadena numérica o sufijos (ej. .1, _2, etc.) para sub-imágenes/videos
        sufijo = ""
        if len(numeros) > 1:
