...
ORIGEN = "PULSERAS BP"
DESTINO = os.path.join("imagenes", "BP", "pulserasbp")
EXCEL_PATH = "PULSERAS BP.xlsx"
PREFIJO_DESTINO = "pulserasbp"
...
if not os.path.exists(EXCEL_PATH):
    ...
    # Leer los IDs válidos desde el Excel (intentando encabezado estándar o flexible)
    try:
        # Probamos primero leyendo con header=2 o header dinámico
        df = pd.read_excel(EXCEL_PATH, header=2)
        if 'NUMERO' not in [str(c).upper() for c in df.columns]:
            df = pd.read_excel(EXCEL_PATH, header=0)
            
        ids_validos = set()
        for idx, row in df.iterrows():
            col_id = None
            for col in ['NUMERO', 'NUM.', 'ID', 'Id', 'numero', 'Número', 'PULSERAS', 'CODIGO']:
                if col in df.columns:
                    col_id = col
                    break
            
            if not col_id and len(df.columns) > 1:
                col_id = df.columns[1]
...
if not os.path.exists(ORIGEN):
    ...
    for archivo in archivos:
        ...
        # Extraer números del nombre original del archivo (ej. "pulsera_1_2.jpg" -> [1, 2])
        ...
        # Obtener sufijos adicionales (como .1, _2, etc.) para múltiples imágenes o videos por producto
        sufijo = ""
        if len(numeros) > 1:
