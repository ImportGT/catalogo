...
class PDFCatalogoUnificado(FPDF):
    def __init__(self, titulo_general, lista_categorias_cortas):
        ...
        self.lista_categorias_cortas = lista_categorias_cortas  # Lista de tuplas: (nombre_corto, link_obj)
...
def generar_pdf_desde_excel(ruta_excel, nombre_linea, coleccion_carpeta, prefijo_archivo, carpeta_imagenes, mostrar_precios=True, margen_porcentaje=0):
    ...
    for index, row in df.iterrows():
        ...
        if coleccion_carpeta.lower() == "swarovski":
            if "Anillos" in nombre_linea:
                palabra_clave = "anillos_swa"
            elif "Aretes" in nombre_linea:
                palabra_clave = "aretes_swa"
            elif "Pulseras" in nombre_linea:
                palabra_clave = "pulseras_swa"
            else:
                palabra_clave = "collares_swa"
        elif coleccion_carpeta.lower() == "bano_de_plata":
            if "Anillos" in nombre_linea:
                palabra_clave = "anillosbp"
            elif "Aretes" in nombre_linea:
                palabra_clave = "aretessbp"
            elif "Cadenas" in nombre_linea:
    ...
    def obtener_nombre_corto(nombre_largo):
        ...
        if len(partes) > 1:
            if partes[0].lower() == "charms":
                return partes[1]
            elif partes[1].lower() in ["pandora", "swarovski", "baño", "de", "plata"]:
                return partes[0]
            else:
